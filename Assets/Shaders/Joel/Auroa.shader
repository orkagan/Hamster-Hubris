    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag (Interpolator_v2f i) : SV_Target
            {
                ...
                float xOffset = cos(i.uv.y + TAU - 20) - _AnimSpeed * 5;
                t  += tan((i.uv.y * TAU + xOffset * _Time.yy * _AnimSpeed) * _FreeRange / 20) * _FreeRangeB + _FreeRangeB;
               // t += 1-i.uv.y;
               // t /= 3+i.uv.y;
                ...
                float4 o = lerp((t * _ColorA) + (t * _ColorB) / _FreeRangeB, _ColorB, _ColorC) * (waves + color )  * TopBottomRemover;
